...
func (r *UserRepoPG) Create(ctx context.Context, u *models.User) error {
	query := `
		INSERT INTO users (id, customer_id, name, email, password_hash, role, default_project_id, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
	`
	_, err := r.db.Exec(ctx, query, u.ID, u.CustomerID, u.Name, u.Email, u.PasswordHash, u.Role, u.DefaultProjectID)
	return err
}

func (r *UserRepoPG) GetByID(ctx context.Context, id string) (*models.User, error) {
	query := `
		SELECT id, customer_id, name, email, password_hash, role, default_project_id, created_at, updated_at
		FROM users
		WHERE id = $1
		LIMIT 1
	`
...
func (r *UserRepoPG) Update(ctx context.Context, u *models.User) error {
	query := `
		UPDATE users
		SET name = $1, email = $2, password_hash = $3, role = $4, default_project_id = $5, updated_at = $6
		WHERE id = $7 AND customer_id = $8
	`
	_, err := r.db.Exec(ctx, query, u.Name, u.Email, u.PasswordHash, u.Role, u.DefaultProjectID, time.Now(), u.ID, u.CustomerID)
	return err
}
...
func (r *UserRepoPG) CreatePending(ctx context.Context, u *models.User) error {
    query := `
        INSERT INTO users (id, customer_id, email, role, is_pending, created_at, updated_at)
        VALUES ($1, $2, $3, $4, true, NOW(), NOW())
    `
    _, err := r.db.Exec(ctx, query, u.ID, u.CustomerID, u.Email, u.Role)
    return err
}
