...
func (r *ActivityRepoPG) Create(ctx context.Context, a *models.IssueActivity) error {
	...
	_, err := r.db.Exec(ctx, `INSERT INTO issue_activity_logs (id, issue_id, user_id, action, metadata, created_at) VALUES ($1,$2,$3,$4,$5,NOW())`,
...
func (r *ActivityRepoPG) ListByIssue(ctx context.Context, issueID string) ([]models.IssueActivity, error) {
	rows, err := r.db.Query(ctx, `SELECT id, issue_id, user_id, action, metadata, created_at FROM issue_activity_logs WHERE issue_id = $1 ORDER BY created_at ASC`, issueID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var meta json.RawMessage
		if err := rows.Scan(&a.ID, &a.IssueID, &a.UserID, &a.Action, &meta, &a.CreatedAt); err != nil {
			return nil, err
		}
		if len(meta) > 0 {
			var v interface{}
